...
def create_config_gui(default_server=DEFAULT_SERVER_URL):
    ...
    def save_and_close():
        try:
            with open(get_config_path(), 'w') as f: json.dump(APP_CONFIG, f, indent=4)
            setup_startup()
            messagebox.showinfo("Success", "Configuration Verified & Saved!\n\nThe Agent will now start.")
            root.destroy()
            # Reload to show dashboard
            new_c = load_config()
            if new_c: show_status_gui(new_c)
        except Exception as e:
...
if __name__ == "__main__":
    setup_startup() # Ensure registry is set
    
    cfg = load_config()
    is_bg = '--background' in sys.argv
    
    if is_bg and cfg:
        # Case 1: Auto-start (Silent)
        run_agent_process(cfg)
    elif cfg:
        # Case 2: Manual Start (Show Dashboard)
        show_status_gui(cfg)
    else:
        # Case 3: First Run (Show Setup)
